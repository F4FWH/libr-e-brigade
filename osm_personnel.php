...
while ($row = @mysqli_fetch_array($result1)) {
    ...
    // Clé d'identification par coordonnées
    ...
    if (strlen($L_LAT) > 1) {
        ...
        // Met à jour le centrage sur la dernière personne
        $center_lat = $L_LAT;
        $center_lng = $L_LNG;
    }
}

// Génération du JavaScript pour les markers
$map_data = '';
...
<style>
    /* Assurez-vous que les éléments de filtre ont un z-index plus élevé que la carte */
    ...
    /* Assurez-vous que votre menu a un z-index plus élevé que la carte */
    .menu, .menu:hover {
        z-index: 1000; /* Utilisez une valeur suffisamment grande */
        position: relative; /* Assurez-vous que la position est définie */
    }

    /* Assurez-vous que la carte a un z-index inférieur */
    #map_canvas {
        z-index: 1;
    }
</style>
